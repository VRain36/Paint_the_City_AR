...
namespace CustomScripts
{
    public class DatabaseManager : MonoBehaviour
    {
        ...
        public string artlist_API_url = "[TO-DO 2]";
        ...
        private IEnumerator GetMySQLData()
        {
            ...
            for (int i = 0; i < php_result.Length ; i++)
            {
                ...
                artInfo[i].setArtworkID(Int32.Parse(php_result_row[1]));
                artInfo[i].setArtName(php_result_row[2]);
                artInfo[i].setArtworkUrl(php_result_row[3] + "artwork.obj");    
                artInfo[i].setPublicMode(Int32.Parse(php_result_row[5]));
                artInfo[i].setUserID(php_result_row[6]);
